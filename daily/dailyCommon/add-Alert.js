...
export default class AddAlert extends Component {
    constructor(props) {
        ...
        this.state = {
            switchValue:params.timeData.confirm_recept,
            selectNum:params.timeData.selectNum,
            isPickerVisible:false,
            alertTime:params.timeData.alertTime,
    ...
    showAlertTime(){
        let {params}=this.props.navigation.state;
        if(params.timeData.selectNum==8){
            let  stamp=params.timeData.alertTime;
            let date=  new Date(parseInt(stamp) * 1000);
            this.setState({
                showTime:this.formatDate(date)
            })
        }
